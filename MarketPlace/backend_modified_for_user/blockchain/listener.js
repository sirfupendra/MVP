...
if (!RPC_URL || !CONTRACT_ADDRESS) {
  ...
  // ---- JobPosted ----
  contract.on("JobPosted", async (jobId, client, price, jobHash) => {
    try {
  ...
  // ---- JobAccepted ----
  contract.on("JobAccepted", async (jobId, providerAddr) => {
    try {
      const id = jobId.toString();
  ...
  // ---- JobCompleted ----
  contract.on("JobCompleted", async (jobId, resultHash) => {
    try {
      const id = jobId.toString();
  ...
  // ---- JobPaid ----
  contract.on("JobPaid", async (jobId) => {
    try {
      const id = jobId.toString();
